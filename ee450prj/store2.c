...
#define UDPSTORE2 "9319" //UDP Port for listening from store1

#define UDPSTORE2SEND1 "10319" //Port of store2 for sending to store3 during 1st round
#define UDPSTORE3 "13319" //Port of store3 binding for 1st round send

#define UDPSTORE2ROUND2 "11319" //UDP Port for listening from store1 

#define UDPSTORE2SEND3 "12319" //UDP Port for sending to store3 round2
#define UDPSTORE3ROUND2 "15319" //UDP Port of store3 binding for 2nd round send
...
pFile=fopen("Store-2.txt","r");
if(pFile==NULL)
{
  perror("Error");
}
strcpy(outlet_vector,"2");
...
printf("Phase 1: The outlet vector <%s> for store_2 has been sent to the central warehouse\n",&outlet_vector[2]);

    close(sockfd);

printf("End of Phase1 for store2\n");
...
//receiving from store1
    ...
    buf[numbytes] = '\0';
	printf("Phase 2: Store_2 has UDP port %s and IP address %s\n", UDPSTORE2, inet_ntoa(server_addr.sin_addr));
    printf("Phase 2: Store_2 received the truck-vector<%s> from the store1 \n",&buf[2]);
    close(sockfd);
...
//Sending to store3
	...
	if ((rv1 = getaddrinfo("localhost", UDPSTORE2SEND1, &hints, &servinfo)) != 0) 
	{
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
	}
	...
	printf("Phase 2: The Store2 has UDP port number %s and IP address %s\n", UDPSTORE2SEND1, inet_ntoa(server_addr.sin_addr));

    printf("Phase 2: The updated truck-vector<%s> has been sent to store_3 \n",&truck_vector[2]);
    printf("Phase 2: Store_2 updated outlet-vector is <%s>\n",&outlet_vector[2]);

    close(sockfd);

///////////////////////////////////////////////////////////////////////
//receiving from store1 - round2
...
memset(&hints, 0, sizeof hints);
    ...
    printf("Phase 2: Store_2 has UDP port %s and IP address %s\n", UDPSTORE2ROUND2, inet_ntoa(server_addr.sin_addr));
...
memset(&hints, 0, sizeof hints);
	...
	//testing
	if ((rv1 = getaddrinfo("localhost", UDPSTORE2SEND3, &hints, &servinfo)) != 0) 
	{
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
	}
	...
	printf("Phase 2: The Store2 has UDP port number %s and IP address %s\n", UDPSTORE2SEND3, inet_ntoa(server_addr.sin_addr));
    ...
    printf("End of Phase 2 for store 2\n");
